function output = mixedSingle(im_s, mask_s, im_background)
    ...
    four_snow = four_center(snow);
    four_back = four_corner(snow);
    
    mix_mask = (abs(four_snow) - abs(four_pen) > 0);
    four_pen(mix_mask) = four_snow(mix_mask);
    
    four_grad = four_pen + four_back .* four_mask;
